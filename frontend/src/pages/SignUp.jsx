...
const SignUpPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Add user to Firestore using user.uid as document ID
      await setDoc(doc(db, "users", user.uid), {
        uid: user.uid,
        email: user.email,
        name: formData.name,
        username: formData.name.toLowerCase().replace(/\s+/g, '') || user.email.split('@')[0],
        rating: 1200,
        solvedProblems: 0,
        rank: 'Newbie',
      ...
      console.log('User created in Firestore with UID:', user.uid);
      navigate("/login");
    } catch (error) {
  ...
  const handleGoogleSignUp = async () => {
    ...
    try {
      ...
      // Add user to Firestore using user.uid as document ID (Google sign up)
      await setDoc(doc(db, "users", user.uid), {
        uid: user.uid,
        email: user.email,
      ...
      console.log('Google user created in Firestore with UID:', user.uid);
      navigate("/login");
    } catch (err) {
